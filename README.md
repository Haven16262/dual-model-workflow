...
两个会话还会按项目目录自动命名(通过 `--name`):项目在 `~/projects/bili` 下,两个会话就叫 `bili-overseer` 和 `bili-worker`。名字带项目前缀是有意的——并行开两个项目时,固定叫 `worker` 会导致交接消息投递到**另一个项目**的工作者那里,比没有自动化更糟。
...
### 让会话自己通知对方(Linux / macOS)

上面那两步「你走到另一个终端敲字」可以省掉。Claude Code 2.1.224 起支持跨会话消息(`SendMessage` + `ListAgents`),两个终端同时开着时,写完 `context.md` 的一方直接通知对方,对方按 `/as-worker` / `/as-overseer` 的完整流程接手。

会话名由启动脚本按项目目录设定,所以**发送方不需要查询就知道对方叫什么**——在 `bili` 项目里的全局者,直接发给 `bili-worker` 即可。
...
发送失败(对方没开终端、平台不支持、名字对不上)时,**回退到人工中转,不重试**。所以这是可选加速,不是新的必经路径:自动通道断了,工作流退化成原来的样子,照常运转。

**平台限制:** 跨会话消息目前只在 macOS 和 Linux 上可用,Windows 端还没有。Windows 用户不需要改任何配置——发送会失败,规则自动回退人工,行为等同于本节不存在。因此模板不为平台分叉,`windows/dual-model.ps1` 也不设置会话名。
